...
std::string Quad::toString(bool verbose)
{
	...
	size_t spaces;
	if (res.length() > labelSpace) {
		spaces = 0;
	} else {
		spaces = labelSpace - res.length();
	}
...
std::string BinOpQuad::repr()
{
	...
	return dst->valString() + " := " + src1->valString() + " " +
	       BinOpQuad::oprString(opr) + " " + src2->valString();
}

UnaryOpQuad::UnaryOpQuad(Opd *dstIn, UnaryOp opIn, Opd *srcIn)
	: dst(dstIn), opr(opIn), src(srcIn)
{
	assert(dstIn != nullptr);
	assert(srcIn != nullptr);
}
...
std::string UnaryOpQuad::repr()
{
	...
	return dst->valString() + " := " + opString + src->valString();
}
...
JmpQuad::JmpQuad(Label *tgtIn) : Quad(), tgt(tgtIn)
{
}
...
JmpIfQuad::JmpIfQuad(Opd *cndIn, Label *tgtIn) : Quad(), cnd(cndIn), tgt(tgtIn)
{
}
...
GetRetQuad::GetRetQuad(Opd *opdIn) : Quad(), opd(opdIn)
{
}
...
SetArgQuad::SetArgQuad(size_t indexIn, Opd *opdIn) : index(indexIn), opd(opdIn)
{
}
...
GetArgQuad::GetArgQuad(size_t indexIn, Opd *opdIn) : index(indexIn), opd(opdIn)
{
}
...
SetRetQuad::SetRetQuad(Opd *opdIn) : opd(opdIn)
{
}
...
{
	std::string res = dst->locString() + " := " + src->locString() +
			  " ADD64 " + off->valString();
	return res;
}
